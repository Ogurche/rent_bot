...
async def new_take (message: types.Message):
    if message.text == 'Посмотреть мой запрос':
        ...
        if  len(check_orders_fr_id) > 0:
            await message.answer('У вас уже есть активное объявление', reply_markup =await delete_order_kbd())
        ...
        await message.answer ('Выберите тип жилья', reply_markup = await confirmation_keybord(dictionary=about_house_dict['type_of_house'], additional_b_data='Отмена', additional_b_name='Назад'))
...
async def pass_type_house (callback_query:types.CallbackQuery, state: FSMContext):
    if callback_query.data == 'next':
        await callback_query.message.edit_text('Выберите количество комнат', reply_markup = await take_rooms_keybord(4, '4+', '4+', 'Назад', 'back'))
        ...
        await callback_query.message.edit_text(f'Выбрать еще один вариант?', reply_markup = await confirmation_keybord(dictionary=about_house_dict['type_of_house'], additional_b_data='next', additional_b_name='Далее'))
        await make_order.type_of_house.set()
...
async def pass_rooms (callback_query:types.CallbackQuery, state: FSMContext):
    if callback_query.data == 'back':
        await callback_query.message.edit_text('Выберите тип жилья', reply_markup = await confirmation_keybord(dictionary=about_house_dict['type_of_house'], additional_b_data='Отмена', additional_b_name='Назад'))
        await state.finish()
        ...
        await callback_query.message.edit_text('Выбрать еще один вариант?', reply_markup = await take_rooms_keybord(4, '4+', '4+', "Дальше",'next'))
        await make_order.rooms.set()
    ...
    if message.text == 'Назад':
        ...
        async with state.proxy() as data_take:
            if 'price_min' in data_take:
                ...
                del data_take['rooms']
        await message.answer('Выберите количество комнат', reply_markup = await take_rooms_keybord(4, '4+', '4+', 'Назад', 'back'))
        dot_msg = await bot.send_message (chat_id=message.chat.id,text='.', reply_markup=ReplyKeyboardRemove())  
        await bot.delete_message (chat_id=message.chat.id, message_id=dot_msg.message_id)
        await make_order.rooms.set()
...
async def pass_district (callback_query:types.CallbackQuery, state: FSMContext):
    if callback_query.data == 'back':
        ...
        await callback_query.message.edit_text('Выбрать еще один вариант?', reply_markup = await district_keybord(about_house_dict['district'], 'Далее', 'next'))
        await make_order.district.set()
...
async def pass_type_repair (callback_query:types.CallbackQuery, state: FSMContext): 
    if callback_query.data == 'back':
        ...
        await callback_query.message.edit_text('Выбрать еще один вариант?', reply_markup = await district_keybord(about_house_dict['type_repair'], 'Далее', 'next'))
        await make_order.type_repair.set()
        ...
        async with state.proxy() as data_take:
            if 'area_min' in data_take:
                if 'area_max'not in data_take:
                    ...
                    dot_msg = await bot.send_message (chat_id=message.chat.id,text='.', reply_markup=ReplyKeyboardRemove())  
                    await bot.delete_message (chat_id=message.chat.id, message_id=dot_msg.message_id)
                    await message.answer ("Какой должен быть этаж?", reply_markup = await take_floor_keybord(back='Назад', back_data='back', skip='Пропустить', skip_data='next'))
    ...
    if callback_query.data == 'back':
        ...
        async with state.proxy() as data_take:
            if 'floor'not in data_take:
                data_take['floor'] = '0'
        await callback_query.message.edit_text('Вы ищите квартиру в которую можно с', reply_markup = await kids_animals_take_keybord(back='Назад', back_data='back', skip='Пропустить', skip_data='next'))
        ...
        await callback_query.message.edit_text('Выбрать еще один вариант?', reply_markup = await take_floor_keybord(skip='Далее', skip_data='next'))
        await make_order.floors.set()
...
async def pass_animals_kids (callback_query:types.CallbackQuery, state: FSMContext):
    if callback_query.data == 'back':
        ...
        await callback_query.message.edit_text('Какой должен быть этаж?', reply_markup = await take_floor_keybord(back='Назад', back_data='back', skip='Пропустить', skip_data='next'))
        await make_order.floors.set()
    elif callback_query.data == 'next':
        async with state.proxy() as data_take:
